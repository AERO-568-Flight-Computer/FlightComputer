...
class StirlingInceptor():
    ...
    def __init__(self):
        ...
        self.rxSockStatus.bind(('',
                                self.UDP_PORT_STATUS))  # TODO: not working if NGI is not on network - "only one usage of each socket address is normally permitted

        self.rxSockLimRot = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
        self.rxSockLimRot.bind(('',
                                self.UDP_PORT_LIMROT))  # TODO: not working if NGI is not on network - "only one usage of each socket address is normally permitted
        ...
        while not ROLL_CAL_COMPLETE and not PITCH_CAL_COMPLETE:
            ...
            if axis == 0 and not PITCH_CAL_COMPLETE:  # pitch axis TODO: add extra check for position is close to 0
                ...
                if pitchForce >= OFFSET_TOL:
                    ...
                    print(
                        f"pitch force: {pitchForce} | bias decrease | force bias: {self.PITCH_FORCE_BIAS} | position: {pos[0]}")
                    ...
                    print(
                        f"pitch force: {pitchForce} | bias increase | force bias: {self.PITCH_FORCE_BIAS} | position: {pos[0]}")
                else:
                ...
                if rollForce >= OFFSET_TOL:
                    ...
                    self.txSock.sendto(self.msg07(axis='roll', forceBias=self.ROLL_FORCE_BIAS),
                                       (self.UDP_IP_NGI, self.UDP_PORT_CTLCFG))  # Force Bias Ctrl Msg
                    print(
                        f"roll force: {rollForce} | bias decrease | force bias: {self.ROLL_FORCE_BIAS} | position: {pos[0]}")
                    ...
                    self.txSock.sendto(self.msg07(axis='roll', forceBias=self.ROLL_FORCE_BIAS),
                                       (self.UDP_IP_NGI, self.UDP_PORT_CTLCFG))  # Force Bias Ctrl Msg
                    print(
                        f"roll force: {rollForce} | bias increase | force bias: {self.ROLL_FORCE_BIAS} | position: {pos[0]}")
...
<<<<<<< HEAD
        ba = ba + bytearray(struct.pack("L", 1))  # bytes 33 - 36: Inceptor Number % does this need to be an i
=======
        ba = ba + bytearray(struct.pack("i", 1))  # bytes 33 - 36: Inceptor Number
>>>>>>> 2155676eec43f2134ee966314571027de7d01171
    ...
    def configSetup(self):
        ...
        self.txSock.sendto(self.msg07(axis='roll', forceBias=self.ROLL_FORCE_BIAS),
                           (self.UDP_IP_NGI, self.UDP_PORT_CTLCFG))
